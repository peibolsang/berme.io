...
- What went wrong after isolated publishing: the local loose `origin/main` and `origin/codex/issue-60-scrollytelling` refs named remote SHAs whose objects had never been downloaded into the workspace, so ordinary Git commands reported invalid remote-tracking objects and rejected the next push as non-fast-forward.
- What fixed it safely: preserve local `main` with a backup branch, delete only the two broken remote-tracking refs, fetch them normally, and rebase the two local commits onto the deployed cleanup commit. This rebuilt a valid object graph, preserved every repository change, and allowed a normal fast-forward push without force.

### Issue 73 scrollytelling (2026-08-07)

- What worked: turning the article's `explore > crystallize > reuse > explore higher` operating model into a physics-inspired phase chamber made the ordered argument visible without reusing the existing constraint-layer, route, or workbench metaphors.
- What worked for copy: each beat names the concrete work—prompts and prototypes, repeated dashboard reviews, APIs and schemas, permissions, tests, and pipeline checks—so the argument remains complete without animation.
- What went wrong: the installed `gh` client was too old for JSON or API commands and its saved credentials returned 401; the repository's configured `GITHUB_TOKEN` plus REST remained the reliable read path.
- What went wrong in verification: the first dev-server attempt collided with an existing `.next` lock, and the available `agent-browser` binary was missing.
- What I corrected: used an isolated temporary checkout bound only to `127.0.0.1`, injected the candidate body only into that temporary local fetch path, and verified it with the in-app browser before removing the hook.
- What browser verification confirmed: all four `certainty-phase-change` states activate at 1440×900 and 390×844, active mobile prose can sit below the sticky stage, no component or document overflow appears, and the console and framework-overlay checks stay clean.
- What the old workflow incorrectly gated: it treated renderer deployment as a prerequisite for updating unpublished Issue #73, even though an Issue without the `published` label cannot render as a public post.
- What the user explicitly overrode: they asked to skip deployment and page checks and publish immediately. The REST update inserted only the validated `certainty-phase-change` fence into Issue #73, and the re-fetched body passed stdin validation with exactly one interactive block.
...
### Issue 74 scrollytelling (2026-08-23)

- What worked: expressing the article's maturity-amplifier argument as one autonomy control feeding an explicit and an implicit organizational circuit made the adoption gap visible without reusing the existing loop, constraint, workbench, or phase-change metaphors.
- What worked for copy: the five beats name the shared tool, APIs and schemas, scoped permissions, tests and production feedback, tribal knowledge, ambiguous ownership, the widening review queue, and the platform defaults that can turn expert practice into common practice.
- What browser verification caught: the first desktop activation target placed prose behind the sticky stage, CSS transforms on positioned SVG output groups overrode their coordinate transforms, and dark-mode article ink made labels disappear on the permanently light instrument surface.
- What corrected those issues: moved the desktop activation target to `0.82`, kept output-meter changes to opacity and color, and pinned the instrument's label ink to its own dark token.
- What worked for motion: occasional explanatory state changes use retargetable CSS transitions; the autonomy needle and narrative overlays use the shared strong curves, while reduced motion removes every positional change and retains 160ms opacity feedback.
- What browser verification confirmed: all five states activate at 1440×900 and 390×844, active prose remains fully readable below the sticky stage, the document and component have no horizontal overflow, forced reduced-motion styles apply correctly, and the console stays clean.
- What publication safety confirmed: the authenticated REST GET/PATCH/GET flow inserted `adoption-amplifier` once after the maturity-amplifier paragraph, preserved every byte outside the new fence, and the refetched Issue #74 body passed stdin validation.
- What the user corrected: the red waveform obscured the three implicit-team modules without adding meaning, the autonomy beat changed only a dial and a cryptic edge label, and the platform-defaults beat repeated the prose section that followed it.
...
### Issue 75 scrollytelling (2026-09-04)

- What worked: treating the large essay as three visual arguments produced a main artifact-plus-evidence loop, an editorial proof desk for the article workflow, and a cinematic proof room for the Remotion workflow instead of stretching one generic pipeline across the post.
- What worked for copy: all eighteen beats name a concrete source, artifact, check, decision, repair, or consequence; the human-writing pass removed stock corrective formulas while preserving Pablo's practical systems voice.
- What worked for design: the three components share the dark slate instrument language and use amber only for the current state, while their signatures remain distinct—paired release rails, a manuscript with attached proof, and a film frame with parallel inspection meters.
- What worked for motion: occasional explanatory state changes use retargetable CSS opacity and transform transitions with the established strong curves; reduced motion removes positional changes and retains short state feedback.
- What browser verification caught: the persistent video subject protruded below the storyboard cards in the second state. Crossfading that subject out when the storyboard appears removed the overlap without changing the visual world.
- What browser verification confirmed: the three six-step stories activate correctly at desktop and 390×844, light and dark treatments remain readable, every component stays inside the document width, and the temporary standalone harness reports no browser errors.
- What publication safety confirmed: the authenticated REST update inserted `confidence-evidence-loop` once, replaced the two Mermaid fences with `article-evidence-workbench` and `cinematic-proof-room`, preserved the remaining Issue #75 body exactly, and left no Mermaid fences behind.
- What final validation confirmed: the refetched Issue body exactly matched the candidate and passed stdin validation with three interactive blocks; repository validation, TypeScript, lint, and diff checks also passed with only the eight existing image warnings.
- What stayed intentionally absent: no persistent local Markdown article or preview route was created; visual checks used a disposable `/tmp` bundle rather than changing the content model.
- What the user caught after publication: the confidence instrument's rail, station, and loop labels were too small and too dim at the rendered desktop size. Increasing the SVG type scale by roughly one third and raising completed/future-state contrast kept the six-column geometry while making the labels readable.
- What the same typography pass revealed: the article proof desk and cinematic proof room used the same micro-label assumption. Their structures stayed untouched; larger HTML/SVG labels, tighter tracking, and stronger secondary contrast made the source cards, evidence rail, storyboard captions, inspection meters, and gate states legible without crowding the diagrams.
- What the larger confidence labels exposed: the moving evidence card occupied the same band as the upper station names, while both rail headings collided with the first station. Lifting only the artifact rail into its unused headroom and moving the headings above their geometry restored every label without changing the packet, evidence rail, or six-stage composition.
